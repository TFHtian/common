...
import com.tian.lib_common.lib_network.state.paresException
import com.tian.lib_common.lib_network.state.paresResult
...
fun <T> BaseViewModel.request(
    ...
    return viewModelScope.launch {
        runCatching {
            if (isShowDialog) postShowLoadingEvent(loadingType, loadingMessage)
            //请求体
            block()
        }.onSuccess {
            //网络请求成功 关闭弹窗
            postDismissLoadingEvent(loadingType)
            runCatching {
